...
export const notebookDiscount: ChallengeDefinition = defineChallenge<
  NotebookModel,
  DevelopmentParams
>({
  ...
  generate({ rng, difficulty }) {
    ...
    const cashAvailableMinor =
      cheapest + rng.nextInt(0, Math.max(1, instalmentTotal - cheapest - 1000))
  ...
  verify(model) {
    ...
    if (distinct.size !== model.offers.length) {
      issues.push('two offers cost exactly the same, which misleads the player')
    }
