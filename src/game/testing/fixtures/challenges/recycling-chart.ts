...
export const recyclingChart: ChallengeDefinition = defineChallenge<
  RecyclingModel,
  DevelopmentParams
>({
  ...
  generate({ rng }) {
    ...
    const thirdKilograms = rng.nextInt(
      ...
      Math.max(2, Math.min(bestKilograms - 1, thirdStudents * (bestRatio - 1))),
    )
  ...
  evaluate(
    ...
    const chosen = model.courses.find((course) => course.id === answer.optionId)
    if (chosen === undefined) {
      return err({
        kind: 'invalid-answer',
