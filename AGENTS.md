...
- Una decision integrada declara su madurez, y **la madurez es parte de la decision**: `LOCKED`, `PRODUCT DIRECTION`, `RECOMENDADA`, `TEACHER GATE`, `OPEN` o `DEFERRED`. El registro unico es [docs/07-reference/decision-register.md](docs/07-reference/decision-register.md), y lo que sigue sin decidir vive en [preguntas abiertas](docs/07-reference/open-questions.md). Una constante `RECOMENDADA` o `TEACHER GATE` no se escribe como numero magico: se escribe como politica versionada. Ver [ADR-018](docs/03-architecture/adr/ADR-018-blueprint-v0-2-decision-authority.md).
- Presente y objetivo se leen separados: [game engine](docs/03-architecture/game-engine.md) describe el motor implementado, y [la arquitectura objetivo](docs/03-architecture/target-engine-architecture.md) describe lo que falta con el estado real de cada capacidad. No describas en presente algo que no existe.
- `docs/sources/` guarda paquetes externos congelados. No son canonicos y no se editan; su procedencia y que documento absorbio cada cosa esta en [la integracion del blueprint](docs/07-reference/blueprint-v0.2-integration.md).
